...
export default function AddGiftSetPage() {
  ...
  const save = async () => {
    ...
    if (imageFiles.length === 0) return setError("Choose at least one image.");
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error ?? "Could not save gift set.");
        return;
      }
      ...
      <div className="flex flex-col gap-4 px-5 py-5">
        <label className="flex flex-col gap-1.5">
          ...
          <input
            ...
            placeholder="e.g. Mid-Autumn Treats Box"
        ...
        <label className="flex flex-col gap-1.5">
          ...
          <input
            ...
            className="rounded-md border border-beige px-3 py-2.5 text-[14px] text-brown"
          />
          <span className="text-[11px] text-soft-brown">Lower numbers show first on the Gift Box page.</span>
        </label>
        ...
        <label className="flex flex-col gap-1.5">
          ...
          <textarea
            ...
            rows={3}
            placeholder="e.g. 8 assorted cookies in premium gift box"
