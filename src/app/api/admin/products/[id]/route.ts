...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Only trust keepImages entries that actually existed — ignore anything else the client sent.
  ...
  try {
    uploadedUrls = await Promise.all(newImages.map((img) => uploadProductImage(supabase, id, img)));
  } catch (err) {
